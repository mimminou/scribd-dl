...
import { setProgress } from "../../run.js";
...
class ScribdDownloader {
	...
	async execute(url, flag) {
		...
		if (url.match(scribdRegex.DOCUMENT)) {
			filePath = await fn(`https://www.scribd.com/embeds/${scribdRegex.DOCUMENT.exec(url)[2]}/content`)
		} else if (url.match(scribdRegex.EMBED)) {
			filePath = await fn(url)
		} else {
	...
	async embeds_default(url) {
		...
		if (m) {
			...
			while (cur + clientHeight < height) {
				...
				setProgress(Math.floor(((cur + clientHeight) / height) * 100))
			}
			bar.stop();
			setProgress(100)
	...
	async embeds_image(url) {
		...
		if (m) {
			...
			for (let i = 0; i < doc_pages.length; i++) {
				...
				bar.update(i + 1);
				setProgress(Math.floor(((i + 1) / doc_pages.length) * 100))
